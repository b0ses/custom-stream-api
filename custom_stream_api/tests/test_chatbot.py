...
from custom_stream_api.chatbot.models import Badges, BADGE_NAMES
...
def import_aliases(session):
    ...
    TEST_ALIASES = [
        AliasFactory(alias="chat_alert", badge="chat", command="!alert test_text_1"),
...
def import_timers(session):
    global TEST_TIMER_DICTS

    TEST_TIMERS = [
        TimerFactory(command="!echo test command", interval=2),
        TimerFactory(command="!echo test command 2", interval=3),
    ]
        ...
        with mock.patch.object(twitchbot.TwitchBot, "on_pubmsg", new=fake_on_pubmsg):
            with mock.patch.object(twitchbot.TwitchBot, "running", new=fake_running):
                bot = twitchbot.TwitchBot(
                    "test_id", "test_botname", "test_client_id", "test_token", "test_channel", timeout=0.1
                )
                bot.responses = []
...
def simulate_chat(bot, user_name, message, badges):
    tags = {"display-name": user_name, "badges": ",".join([badge.value for badge in badges])}
...
# MAIN COMMANDS
def test_id(chatbot):
    simulate_chat(chatbot, "test_user", "!id", [Badges.GLOBAL_MODERATOR])
    expected_responses = []
    assert chatbot.responses == expected_responses

    simulate_chat(chatbot, "test_user", "!id", [Badges.BROADCASTER])
    expected_response = "Chatbot ID - test_id"
    assert chatbot.responses[-1] == expected_response


def test_echo(chatbot):
    simulate_chat(chatbot, "test_user", "!echo test test 1 2 3", [Badges.VIP])
    expected_responses = []
    assert chatbot.responses == expected_responses

    simulate_chat(chatbot, "test_user", "!echo test test 1 2 3", [Badges.BROADCASTER])
    expected_response = "test test 1 2 3"
    assert chatbot.responses[-1] == expected_response


def test_chatbot(chatbot):
    simulate_chat(chatbot, "test_user", "!shoutout test_user", [Badges.SUBSCRIBER])
    expected_responses = []
    assert chatbot.responses == expected_responses

    simulate_chat(chatbot, "test_user", "!shoutout test_user", [Badges.VIP])
    expected_response = (
        "Hey I know test_user! You should check'em out and drop a follow " "- https://www.twitch.tv/test_user"
    )
    assert chatbot.responses[-1] == expected_response
...
def test_get_commands(chatbot):
    ...
    simulate_chat(chatbot, "test_user", "!get_commands", badge_level)
    expected_response = (
        "Commands include: disconnect, echo, get_alert_commands, get_aliases, get_commands, "
...
def test_random(chatbot):
    simulate_chat(chatbot, "test_user", "!random a b c", [Badges.SUBSCRIBER])
    expected_responses = []
    assert chatbot.responses == expected_responses

    simulate_chat(chatbot, "test_user", "!random a", [Badges.VIP])
    expected_response = "Format: !random option1 option2 [option3 ...]"
    assert chatbot.responses[-1] == expected_response

    simulate_chat(chatbot, "test_user", "!random a b c", [Badges.VIP])
...
def test_spongebob(chatbot):
    simulate_chat(chatbot, "test_user", "!spongebob", [Badges.SUBSCRIBER])
    expected_response = "Format: !spongebob message"
    assert chatbot.responses[-1] == expected_response

    simulate_chat(chatbot, "test_user", "!spongebob stop mimicking me", [Badges.SUBSCRIBER])
...
def test_taco(chatbot):
    simulate_chat(chatbot, "test_user", "!taco test_user2", [Badges.CHAT])
    expected_response = []
    assert chatbot.responses == expected_response

    simulate_chat(chatbot, "test_user", "!taco test_user2", [Badges.SUBSCRIBER])
    expected_response = ["/me test_user aggressively hurls a :taco: at test_user2", "test_user2_tacos: 1"]
    assert chatbot.responses == expected_response

    simulate_chat(chatbot, "test_user", "!taco  ", [Badges.SUBSCRIBER])
...
def test_get_aliases(import_aliases, chatbot):
    ...
    simulate_chat(chatbot, "test_user", "!get_aliases", badge_level)
    expected_response = (
        "Commands include: broadcaster_test_alias, chat_alert, chat_test_alias, mod_test_alias, "
...
def test_import_export_timers(import_timers):
    assert timers.list_timers() == TEST_TIMER_DICTS


def test_remove_timer(import_timers):
